...
router.get('/:code', async (req, res) => {
  ...
  res.json({
    ...
    lifafa: {
      ...
      channels:       l.channels,
      creator_name:   l.creator_id?.name || 'Unknown',
      creator_mobile: l.creator_mobile,
...
router.post('/claim', async (req, res) => {
  ...
  try {
    ...
    // referred_by = mobile of the person who shared their refer link
    ...
    if(lifafa.type === 'toss' && (!guess || guess.toUpperCase() !== lifafa.toss_answer.toUpperCase())) {
      await Transaction.create([{
        receiver_id: user._id, amount: 0, remark: rem, type: 'transfer', status: 'failed'
      }], { session });
      await session.commitTransaction();
    ...
    if(lifafa.type === 'scratch') {
      amt = Math.floor(
        Math.random() * (lifafa.max_range * 100 - lifafa.min_range * 100 + 1) + lifafa.min_range * 100
      ) / 100;
    }
    ...
    let referBonus = 0;
    if(
      lifafa.refer_enabled &&
      lifafa.refer_amt > 0 &&
      referred_by &&
      referred_by !== mobile  // self-refer block
    ) {
      const referrer = await User.findOne({ mobile: referred_by });
      if(referrer) {
